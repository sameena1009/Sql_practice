...
sql-practice/
│
├── 01_Basic_SELECT.sql              # SELECT, column selection, basic retrieval
├── 02_WHERE_clause.sql              # Filtering: comparisons, AND/OR, IN, LIKE, BETWEEN
├── 03_ORDER_BY.sql                  # Sorting ascending/descending, multi-column sort
├── 04_DISTINCT.sql                  # Unique value extraction
├── 05_LIMIT.sql                     # Limiting result sets, Top-N queries
├── 06_Aggregate_Functions.sql       # COUNT, SUM, AVG, MIN, MAX
├── 07_GROUP_BY_AND_HAVING.sql       # Grouped aggregation with filtering
├── 08_STRING_FUNCTIONS.sql          # UPPER, LOWER, TRIM, CONCAT, REPLACE, SUBSTRING_INDEX
├── 09_DATE_FUNCTIONS.sql            # YEAR, MONTH, DATEDIFF, TIMESTAMPDIFF, DATE_FORMAT
├── 10_CASE.sql                      # Conditional logic & categorization
├── 11_SUBQUERY.sql                  # Scalar, correlated & nested subqueries
├── 12_WINDOW_FUNCTIONS.sql          # ROW_NUMBER, RANK, LAG/LEAD, running totals
├── 13_JOINS.sql                     # INNER, LEFT, RIGHT, SELF, multi-table joins
├── 14_DATA_CLEANING.sql             # Real-world data cleaning workflow
├── messy_customer_data_200_rows.sql # Raw messy dataset used for cleaning practice
└── README.md
```
...
2. Load `messy_customer_data_200_rows.sql` first to create the practice database and table.
